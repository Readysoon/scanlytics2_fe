															...
															{#each kneejsonData.sections as items (items)}
																{#if ItemToggle != null && ItemToggle.name == items.name}
																	<div
																		...
																		optionExamToggle == false ||
																		optionStudyInfoToggle == false ||
																		OptPatientStateToggle == false
																			? '60%' //was before 56
																			: '42%'};"
																	>
																			...
																			{#if items.name == 'Findings'}
																				{#if stateobj}
																					<div class="stateObjItem-contentLayer">
																						{#each stateobj as stateObjItem (stateObjItem)}{/each}
																					</div>
																				{/if}
																				<div>
																					<img src="robo2.png" alt="widget" class="robologo" />
																				</div>
																			{/if}
																		</div>
...
<style>
	...
	@media (min-width: 1280px) {
		...
		/* Content Section - Report - findings */
		.aicontentSectionContent {
			height: 94%;
			width: 100%;
			display: flex;
